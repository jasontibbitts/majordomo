...
sub suggest_mta {
  ...
  # Do qmail stuff; all configuration is done at install time so we have no
  # reason to call createlist.
  if ($config->{mta} eq 'qmail') {
    ...
    print QD "|$config->{'install_dir'}/bin/mj_shell -d $dom -Q\n";
    close QD;
    return;
  }

  # Now do sendmail by calling createlist-regen.
